	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for key, value := range expectedHeaders {
				got := headers[key]
				if len(got) == 0 {
					t.Errorf("Expected header %s to be present, but it was missing", key)
					continue
				}
				if !strings.Contains(got[0], value[0]) {
					t.Errorf("Expected %s to equal %s got %s", key, value[0], got[0])
				}
			}
		})
	}
